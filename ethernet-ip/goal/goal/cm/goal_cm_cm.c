...
#include "goal_cm_cm.h"


/****************************************************************************/
/* Local prototypes */
/****************************************************************************/
static GOAL_STATUS_T goal_cmCmModReg(
    void
);

static GOAL_STATUS_T goal_cmCmModAdd(
    void
);
...
static GOAL_CM_MODDEF_T cmMod = GOAL_CM_MODDEF(GOAL_CM_CM_MOD_ID, GOAL_CM); /**< CM module definition */

/* generate 'GOAL_CM_VARENTRY_T cmVars[]' array that maps the above table */
#include <goal_cm_t.h>
GOAL_CM_VARLIST(cmVars, GOAL_CM_CM_VARS);


/****************************************************************************/
/** cm cm variables registration
 *
 */
...
)
{
    ...
    res = goal_mainStageReg(GOAL_STAGE_CM_MOD_REG, &stageCmModReg, GOAL_STAGE_INIT, goal_cmCmModReg);

    /* add callback to configuration management module add stage */
    if (GOAL_RES_OK(res)) {
        res = goal_mainStageReg(GOAL_STAGE_CM_MOD_ADD, &stageCmModAdd, GOAL_STAGE_INIT, goal_cmCmModAdd);
    }
...
)
{
    ...
    if (GOAL_RES_OK(res)) {
        res = goal_cmRegVarVirtual(2, CM_CM_VAR_SAVE, GOAL_CM_UINT8, 1, NULL, goal_cmCmSave);
    }
