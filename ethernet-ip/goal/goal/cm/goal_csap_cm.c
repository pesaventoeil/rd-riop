...
/* Local prototypes */
/****************************************************************************/
static GOAL_STATUS_T goal_csapCmModReg(
    void
);

static GOAL_STATUS_T goal_csapCmModAdd(
    void
);
...
static GOAL_CM_MODDEF_T cmMod = GOAL_CM_MODDEF(GOAL_CM_CSAP_MOD_ID, GOAL_CSAP); /**< CM module definition */

/* generate 'GOAL_CM_VARENTRY_T cmVars[]' array that maps the above table */
#include <goal_cm_t.h>
GOAL_CM_VARLIST(cmVars, GOAL_CM_CSAP_VARS);


/****************************************************************************/
/** csap cm variables registration
 *
 */
...
)
{
    ...
    res = goal_mainStageReg(GOAL_STAGE_CM_MOD_REG, &stageCmModReg, GOAL_STAGE_INIT, goal_csapCmModReg);

    /* add callback to configuration management module add stage */
    if (GOAL_RES_OK(res)) {
        res = goal_mainStageReg(GOAL_STAGE_CM_MOD_ADD, &stageCmModAdd, GOAL_STAGE_INIT, goal_csapCmModAdd);
    }
