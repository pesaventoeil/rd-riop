...
status_t flexspi_nor_write_enable(FLEXSPI_Type *pBase, uint32_t baseAddr)
{
    ...
    flashXfer.port          = FLASH_PORT_USED;
    flashXfer.cmdType       = kFLEXSPI_Command;
    flashXfer.SeqNumber     = 1;
    flashXfer.seqIndex      = NOR_CMD_LUT_SEQ_IDX_WRITEENABLE;

    status = FLEXSPI_TransferBlocking(pBase, &flashXfer);

    return status;
}

status_t flexspi_nor_wait_bus_busy(FLEXSPI_Type *pBase)
{
    /* Wait status ready. */
    bool isBusy;
    uint32_t readValue;
...
status_t flexspi_nor_check_bus_busy(FLEXSPI_Type *pBase, flexspi_handle_t *pHandle, bool *isBusy)
{
    size_t count = 0;
    status_t status;

    status = FLEXSPI_TransferGetCount(pBase, pHandle, &count);
...
status_t flexspi_nor_enable_quad_mode(FLEXSPI_Type *pBase)
{
    flexspi_transfer_t flashXfer;
    status_t status;
    ...
    /* Write enable */
    status = flexspi_nor_write_enable(pBase, 0);
    ...
    status = flexspi_nor_wait_bus_busy(pBase);

    /* Do software reset. */
    FLEXSPI_SoftwareReset(pBase);
...
status_t flexspi_nor_set_read_parameter(
    FLEXSPI_Type *pBase, uint8_t burstLength, bool enableWrap, uint8_t dummyCycle, bool resetPinSelected)
{
    flexspi_transfer_t flashXfer;
    status_t status;
...
status_t flexspi_nor_flash_erase_sector(FLEXSPI_Type *pBase, uint32_t address)
{
    status_t status;
    flexspi_transfer_t flashXfer;
...
status_t flexspi_nor_flash_program(FLEXSPI_Type *pBase, uint32_t dstAddr, const uint32_t *src, uint32_t length)
{
    status_t status;
    flexspi_transfer_t flashXfer;
    ...
    /* Write enable */
    status = flexspi_nor_write_enable(pBase, dstAddr);
    ...
    pBase->AHBCR |= FLEXSPI_AHBCR_CLRAHBRXBUF_MASK | FLEXSPI_AHBCR_CLRAHBTXBUF_MASK;
    pBase->AHBCR &= ~(FLEXSPI_AHBCR_CLRAHBRXBUF_MASK | FLEXSPI_AHBCR_CLRAHBTXBUF_MASK);
#else
    FLEXSPI_SoftwareReset(pBase);
#endif
...
status_t flexspi_nor_flash_page_program(FLEXSPI_Type *pBase, uint32_t dstAddr, const uint32_t *src)
{
    status_t status;
    flexspi_transfer_t flashXfer;
...
status_t flexspi_nor_get_vendor_id(FLEXSPI_Type *pBase, uint8_t *vendorId)
{
    uint32_t temp;
    flexspi_transfer_t flashXfer;
    flashXfer.deviceAddress = 0;
    ...
    status_t status = FLEXSPI_TransferBlocking(pBase, &flashXfer);
...
status_t flexspi_nor_erase_chip(FLEXSPI_Type *pBase)
{
    status_t status;
    flexspi_transfer_t flashXfer;
...
void flexspi_nor_flash_init(FLEXSPI_Type *pBase)
{
    ...
    FLEXSPI_Init(pBase, &config);

    /* Configure flash settings according to serial flash feature. */
    FLEXSPI_SetFlashConfig(pBase, &deviceconfig, FLASH_PORT);
    FLEXSPI_SetFlashConfig(pBase, &deviceconfig, FLASH_PORT2);

    FLEXSPI_EnableIPParallelMode(pBase, true);

    /* Update LUT table. */
    FLEXSPI_UpdateLUT(pBase, 0, tempLUT, CUSTOM_LUT_LENGTH);
